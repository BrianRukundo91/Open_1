...
export class DocumentUploadPage {
  ...
  async getUploadedDocumentNames(): Promise<string[]> {
    const documentSpans = this.page.locator(
      "div:has(> span.truncate) > span.truncate",
    );
    const names: string[] = [];
  ...
  async isDocumentInList(fileName: string): Promise<boolean> {
    ...
    return names.some(
      (name) =>
        name.includes(fileName) || fileName.includes(name.split(".")[0]),
    );
  }
  ...
  async processAllUploadedFiles(): Promise<void> {
    const processButtons = this.page.locator(
      'button:has-text("Process"), button:has-text("Submit"), button:has-text("Analyze")',
    );
   ...
   */
  async verifySupportedFileTypes(
    txtPath?: string,
    pdfPath?: string,
    docxPath?: string,
  ): Promise<void> {
